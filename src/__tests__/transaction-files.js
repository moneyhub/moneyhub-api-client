...
describe('Transaction Files', () => {
  ...
  it('add a transaction file', async () => {
    const { data: file } = await moneyhub.addFileToTransaction({
      userId,
      transactionId,
      fileData: fs.readFileSync(path.join(__dirname, '../../resources/sample.jpg')),
      fileName: 'sample.jpg',
    });
  ...
  it('gets transaction files', async () => {
    const { data: files } = await moneyhub.getTransactionFiles({
      userId,
      transactionId,
    });
  ...
  it('gets a transaction file', async () => {
    const { data: file } = await moneyhub.getTransactionFile({
      userId,
      transactionId,
      fileId,
    });
  ...
  it('deletes a transaction file', async () => {
    const status = await moneyhub.deleteTransactionFile({
      userId,
      transactionId,
      fileId,
    });
