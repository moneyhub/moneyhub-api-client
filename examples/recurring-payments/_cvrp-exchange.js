...
// Must match the client/config used by _cvrp-consent.js (same redirect_uri,
...
// Pass EITHER the raw `code` OR the whole /dev-callback redirect URL.
...
const start = async () => {
  if (!code) {
    console.error("Usage: node -r ts-node/register examples/recurring-payments/_cvrp-exchange.js '<code | full redirect URL>'")
    return
  }
  try {
